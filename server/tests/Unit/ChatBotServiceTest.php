...
it('retrieves history for the authenticated user', function () {
    ...
    $history = (new ChatBotService)->getHistory();
...
it('request in the fastapi server for the chatbot response', function () {
    Http::fake([
        env('CHATBOT_REQUEST_URL') => Http::response([
            'response' => 'This is a chatbot reply.',
        ], 200),
    ]);

    $service = new ChatBotService(); 
    $response = $service->chat('Hello chatbot!');

    $this->assertEquals('This is a chatbot reply.', $response);
});